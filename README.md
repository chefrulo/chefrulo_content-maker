...
## Brand Brain (optional but recommended)

`chef-rulo-brand-brain` (separate local repo, e.g. `/home/eduardo/dev/chef-rulo-brand-brain`) is the canonical source of truth for the brand's voice — positioning, editorial manifesto, British-English writing style, and non-negotiable guardrails (no claims of cultural superiority, no "authentic" as an unsupported verdict, no stereotypes, no generalising a single household as all of Argentina). Point `BRAND_BRAIN_PATH` in `.env.local` at a local clone of that repo and every brief generated by `generate:briefs` will be checked against it. Without it, briefs still generate, just without those guardrails — `npm run doctor` tells you which mode you're in.

That repo also defines **editorial territories** (`knowledge/10-editorial-territories/`, e.g. "Argentine Cooking Techniques", "Family Memory") — concrete subject-matter areas, a different axis from this repo's commercial **brand pillars** (`data/brand.json`, e.g. Product & Craft, Culture & Identity). Every brief tags both, plus a specific `topic` and a `contentPattern` (the structural pattern used, e.g. "Cultural Doorway"):
...
Scrapes recent reels from the accounts in `data/inspiration-accounts.json` (via Apify), then asks Claude to generate 5 reel briefs — one per brand pillar in `data/brand.json` — grounded in what's working in those reels. Each brief also gets an editorial territory, topic and content pattern (see Brand Brain above). If `BRAND_BRAIN_PATH` is set, every brief also has to follow the brand's actual editorial manifesto, writing style and non-negotiable guardrails. Stops at a manual checkpoint: review the generated briefs under `data/briefs/`.
