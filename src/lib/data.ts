...
export async function ensureDataDir(subdir?: string): Promise<void> {
  await mkdir(subdir ? path.join(DATA_DIR, subdir) : DATA_DIR, { recursive: true });
}
...
export async function writeData<T>(filename: string, data: T): Promise<void> {
  ...
  await mutex.runExclusive(async () => {
    await ensureDataDir(path.dirname(filename) === "." ? undefined : path.dirname(filename));
    const filePath = path.join(DATA_DIR, filename);
